...
#'     interval for amplitude.}
#'   \item{phase}{Estimated phase, wrapped to the interval
#'     \code{[0, period)} in the same unit as \code{time_vec}.}
...
#'
#' The confidence interval for amplitude parameter is constructed on the log
#' scale and back-transformed to the original scale via exponentiation to
#' maintain positivity of the amplitude.When the estimated amplitude was
#' numerically close to zero, the phase estimate was not considered well
#' defined and the amplitude confidence interval was considered numerically
#' unstable. In this case, phase, amplitude confidence interval, and
#' phase confidence interval are returned as NA.
#'
#' The confidence interval for the phase parameter is wrapped under circular
#' modulo-\code{period} geometry. Therefore, phase and phase confidence interval
#' values are reported in the interval \code{[0, period)}. For example, when
#' \code{period = 24}, a confidence interval of \code{(-2.5, 4.2)} hours is
#' reported as \code{(21.5, 4.2)} hours, and \code{(18, 26)} hours is reported
#' as \code{(18, 2)} hours. In such cases, the lower bound may be numerically
#' larger than the upper bound, indicating that the interval crosses 0 hours.
#'
#' @references
#' Oehlert, G. W. (1992).
...
params_output <- function(expr_mat, time_vec, period = 24, conf_level = 0.95) {
  ...
  wrap_phase <- function(x) {
    ifelse(is.finite(x), x %% period, NA_real_)
  }
  ...
  for (i in seq_len(n_gene)) {
    ...
    if (is.finite(amp_hat) && amp_hat > amp_tol) {
      ...
      phase_hat_raw <- atan2(b2, b1) / w
      phase_hat <- wrap_phase(phase_hat_raw)
      ...
      if (is.finite(phase_var) && phase_var >= 0) {
        ...
        phase_ci_lower_raw <- phase_hat_raw - tcrit * phase_se_internal
        phase_ci_upper_raw <- phase_hat_raw + tcrit * phase_se_internal

        phase_ci_lower <- wrap_phase(phase_ci_lower_raw)
        phase_ci_upper <- wrap_phase(phase_ci_upper_raw)
      }
    }
