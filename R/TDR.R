#' Test whether rhythmicity differs across multiple conditions.
#'
...
#'
#' # run TDR
#' TDR_res <- TDR(data_list = data_list, timepoint_list = timepoint_list, ncond = 3, period = 24)
#'
#' # view results
#' TDR_res
#'
#' # count genes with significant differential rhythmicity using nominal p-values below 0.05
#' sum(TDR_res$pvalue < 0.05, na.rm = TRUE)
#'
#' # specify the FDR level
#' fdr_level <- 0.05
#'
#' # count genes with significant differential rhythmicity using BH-adjusted p-values
#' sum(TDR_res$BH < fdr_level, na.rm = TRUE)
#'
#' # count genes with significant differential rhythmicity using Storey q-values
#' # qvalue is NA if the qvalue package is not installed
#' sum(TDR_res$qvalue < fdr_level, na.rm = TRUE)
#'
#' # ------------------------------------------------------------
...
#' )
#'
#' amp1_ns <- c(1.2, 1.0, 0.8, 0.6, 0.4, 0.4, 0.2, 0.2)
#' amp2_ns <- c(0.6, 1.4, 0.3, 1.0, 0.4, 0.4, 0.2, 0.2)
#' amp3_ns <- c(1.5, 0.5, 1.1, 0.2, 0.4, 0.4, 0.2, 0.2)
#'
#' phi1_ns <- c(2, 4, 6, 8, 10, 12, 14, 16)
#' phi2_ns <- c(6, 2, 10, 4, 10, 12, 14, 16)
#' phi3_ns <- c(4, 8, 2, 12, 10, 12, 14, 16)
#'
#' mesor1_ns <- rep(1.0, 8)
#' mesor2_ns <- rep(1.3, 8)
#' mesor3_ns <- rep(0.8, 8)
...
#'   mesor1_ns[i] + amp1_ns[i] * cos(2 * pi * (time_vec1_ns - phi1_ns[i]) / period) +
#'     rnorm(length(time_vec1_ns), 0, 0.2)
#' }))
#'
#' data2_ns <- t(sapply(seq_along(amp2_ns), function(i) {
#'   mesor2_ns[i] + amp2_ns[i] * cos(2 * pi * (time_vec2_ns - phi2_ns[i]) / period) +
#'     rnorm(length(time_vec2_ns), 0, 0.2)
#' }))
#'
#' data3_ns <- t(sapply(seq_along(amp3_ns), function(i) {
#'   mesor3_ns[i] + amp3_ns[i] * cos(2 * pi * (time_vec3_ns - phi3_ns[i]) / period) +
#'     rnorm(length(time_vec3_ns), 0, 0.2)
#' }))
#'
#' rownames(data1_ns) <- paste0("gene", seq_along(amp1_ns))
#' rownames(data2_ns) <- paste0("gene", seq_along(amp2_ns))
#' rownames(data3_ns) <- paste0("gene", seq_along(amp3_ns))
#'
...
#'
#' TDR_res_ns <- TDR(data_list = data_list_ns, timepoint_list = timepoint_list_ns,
#'                   ncond = 3, period = 24)
#' TDR_res_ns
#'
#' sum(TDR_res_ns$pvalue < 0.05, na.rm = TRUE)
#'
#' fdr_level <- 0.05
#'
#' sum(TDR_res_ns$BH < fdr_level, na.rm = TRUE)
#'
#' sum(TDR_res_ns$qvalue < fdr_level, na.rm = TRUE)
#'
#' # ------------------------------------------------------------
...
#' n_gene <- 12
#'
#' amp1_miss <- c(1.2, 1.0, 0.8, 0.6, 0.4, 0.4, 0.2, 0.2, 1.1, 0.9, 0.7, 0.5)
#' amp2_miss <- c(0.6, 1.4, 0.3, 1.0, 0.4, 0.4, 0.2, 0.2, 1.3, 0.7, 0.9, 0.4)
#' amp3_miss <- c(1.5, 0.5, 1.1, 0.2, 0.4, 0.4, 0.2, 0.2, 1.0, 0.8, 0.6, 0.3)
#'
#' phi1_miss <- c(2, 4, 6, 8, 10, 12, 14, 16, 3, 5, 7, 9)
#' phi2_miss <- c(6, 2, 10, 4, 10, 12, 14, 16, 5, 7, 9, 11)
#' phi3_miss <- c(4, 8, 2, 12, 10, 12, 14, 16, 1, 3, 5, 7)
#'
#' mesor1_miss <- rep(1.0, n_gene)
#' mesor2_miss <- rep(1.3, n_gene)
#' mesor3_miss <- rep(0.8, n_gene)
#'
#' data1_miss <- t(sapply(seq_len(n_gene), function(i) {
#'   mesor1_miss[i] + amp1_miss[i] * cos(2 * pi * (time_vec1_miss - phi1_miss[i]) / period) +
#'     rnorm(length(time_vec1_miss), 0, 0.2)
#' }))
#'
#' data2_miss <- t(sapply(seq_len(n_gene), function(i) {
#'   mesor2_miss[i] + amp2_miss[i] * cos(2 * pi * (time_vec2_miss - phi2_miss[i]) / period) +
#'     rnorm(length(time_vec2_miss), 0, 0.2)
#' }))
#'
#' data3_miss <- t(sapply(seq_len(n_gene), function(i) {
#'   mesor3_miss[i] + amp3_miss[i] * cos(2 * pi * (time_vec3_miss - phi3_miss[i]) / period) +
#'     rnorm(length(time_vec3_miss), 0, 0.2)
#' }))
#'
#' rownames(data1_miss) <- paste0("gene", seq_len(n_gene))
#' rownames(data2_miss) <- paste0("gene", seq_len(n_gene))
#' rownames(data3_miss) <- paste0("gene", seq_len(n_gene))
#'
...
#'
#' TDR_res_miss <- TDR(data_list = data_list_miss, timepoint_list = timepoint_list_miss,
#'                     ncond = 3, period = 24)
#' TDR_res_miss
#'
#' sum(TDR_res_miss$pvalue < 0.05, na.rm = TRUE)
#'
#' fdr_level <- 0.05
#'
#' sum(TDR_res_miss$BH < fdr_level, na.rm = TRUE)
#'
#' sum(TDR_res_miss$qvalue < fdr_level, na.rm = TRUE)
#'
#' @export

TDR <- function(data_list, timepoint_list, ncond, period = 24) {
  ...
  if (length(unique(row_counts)) != 1) {
    stop("All datasets in 'data_list' must have the same number of rows (genes).")
  }

  p <- unique(row_counts)
  results <- matrix(NA_real_, nrow = p, ncol = 4)

  Lmat <- matrix(0, ncol = 3 * ncond, nrow = 2 * (ncond - 1), byrow = TRUE)
  Lmat[1:(ncond - 1), 2] <- 1
  Lmat[ncond:(2 * (ncond - 1)), 3] <- 1
  for (r in 1:(ncond - 1)) {
    Lmat[r, 3 * r + 2] <- -1
  }
  for (r in ncond:(2 * (ncond - 1))) {
    Lmat[r, 3 * (r - ncond + 1) + 3] <- -1
  }
  nrlma <- nrow(Lmat)
  ...
  for (i in seq_len(p)) {
    ...
    gamma_vec <- rep(0, 3 * ncond)
    var_error_vec <- rep(NA_real_, ncond)
    varcov <- matrix(0, ncol = 3 * ncond, nrow = 3 * ncond)
    ...
    for (myk in seq_len(ncond)) {
      ...
      varcov[k1:k2, k1:k2] <- as.numeric(var_error) * xtxinv
      gamma_vec[k1:k2] <- as.numeric(gamma_hat)
    ...
    newtest100 <- as.numeric(t(diff_vec) %*% Lcov %*% diff_vec)
    ...
    mu1 <- nrlma
    mu2 <- 2 * nrlma
    dummy_varcov <- matrix(0, ncol = 3 * ncond, nrow = 3 * ncond)
    store_var_sigma2_kg <- rep(NA_real_, ncond)
    store_capb_list <- vector("list", ncond)
    ...
    for (k1 in seq_len(ncond)) {
      ...
      dummy_varcov[k2:k3, k2:k3] <- xtxinv_list_gene[[k1]]
      capbk1 <- Lmat %*% dummy_varcov %*% t(Lmat) %*% Lcov

      term2 <- capbk1 %*% capbk1
      term3 <- term2 %*% capbk1
      term4 <- term3 %*% capbk1

      term1_trace <- sum(diag(capbk1))
      term2_trace <- sum(diag(term2))
      term3_trace <- sum(diag(term3))
      term4_trace <- sum(diag(term4))

      store_capb_list[[k1]] <- capbk1

      mu1 <- mu1 + (term2_trace) * 2 * var_error_vec[k1]^2 / deg_frdm

      dummy_varcov[k2:k3, k2:k3] <- 0
    ...
    for (k1 in 1:(ncond - 1)) {
      for (k1new in (k1 + 1):ncond) {
        qnty1 <- (
          store_capb_list[[k1]] %*% store_capb_list[[k1new]] +
            store_capb_list[[k1new]] %*% store_capb_list[[k1]]
        )
        qnty2 <- (
          store_capb_list[[k1]] %*% store_capb_list[[k1]] %*%
            store_capb_list[[k1new]] %*% store_capb_list[[k1new]]
        )
